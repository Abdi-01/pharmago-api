...
module.exports = {
    ...
    addCustomCart: (req, res) => {
        ...
        let sqlAdd = `INSERT INTO tbcartCustom (iduser, qty_kapsul, total_harga)
                    VALUES ( ${cartCustom.iduser}, ${pool.escape(cartCustom.qty_kapsul)}, ${pool.escape(cartCustom.total_harga)});`
        ...
        pool.query(sqlAdd, (err1, results1) => {
            ...
            if (results1) {
                ...
                cartCustom_detail.forEach(item => {
                    data.push(`(null, ${pool.escape(results1.insertId)}, ${pool.escape(item.idproduct)}, ${pool.escape(item.qty_per_satuan)}, ${pool.escape(item.total_price_satuan)}) `)
                })
