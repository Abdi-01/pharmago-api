...
module.exports = {
  addTransaction: (req, res) => {
    ...
    let sqlAdd = `INSERT INTO tbtransaction (iduser, invoice_number, ongkir, total_payment, iduser_address, transaction_type) 
      ...
      invoice
    )}, ${pool.escape(req.body.ongkir)}, ${pool.escape(
      req.body.total_payment
    )}, ${pool.escape(req.body.iduser_address)}, ${pool.escape(
      transaction_type
    )} )`;
        ...
        if (transaction_type === 'QO') {
          req.body.checkout.forEach((item, index) => {
            data.push(
              `(null, ${pool.escape(results1.insertId)}, ${pool.escape(
                item.idproduct
              )}, null, ${pool.escape(item.qty_qo)}, ${pool.escape(
                item.total_price
              )}, ${pool.escape(item.note)} )`
            );
          ...
          req.body.checkout.forEach((item, index) => {
            data.push(
              `(null, ${pool.escape(results1.insertId)}, ${pool.escape(
                item.idproduct
              )}, ${pool.escape(item.qty_co)}, null, ${pool.escape(
                item.total_price
              )}, ${pool.escape(item.note)} )`
            );
            getIdProduct.push(`${pool.escape(item.idproduct)}`);
          ...
          pool.query(sqlGetProduct, (err3, results3) => {
            ...
            results3.forEach((item, index) => {
              ...
              if (transaction_type === 'QO') {
                ...
                update.push(
                  `(${pool.escape(item.idproduct_stock)}, ${pool.escape(
                    item.idproduct
                  )}, ${pool.escape(stockBaru)}, ${pool.escape(
                    item.qty_per_pcs
                  )}, ${pool.escape(item.satuan)}, ${pool.escape(
                    item.total_stock_satuan
                  )}, ${pool.escape(item.price_pcs)}, ${pool.escape(
                    item.price_per_satuan
                  )}, ${pool.escape(item.status)}, ${pool.escape(
                    item.type_obat
                  )} )`
                );
              } else if (transaction_type === 'CO') {
